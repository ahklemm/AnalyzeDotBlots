...
IDnames = createSpotIDArray(); //createSpotIDArray() creates an arrray that contains the names of each spot
...
for(r=0; r<roiManager("Count"); r++){
	...
	//roiManager("Select", r);
	run("Measure");
	ROI_ID = getResult("Mean", 1);
	ROI_ID_norm = ROI_ID - 1;  //ROI-IDs start with 1 --> set to 0
	run("Clear Results");
...
//measure on inverted, background subtracted image
	...
	selectWindow(title);
	run("Subtract Background...", "rolling="+3*spotsize);
	//run("Invert");
